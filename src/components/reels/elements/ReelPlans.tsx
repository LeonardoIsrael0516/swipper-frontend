...
interface ReelPlansProps {
  element: SlideElement;
  onButtonClick?: (url: string, openInNewTab: boolean) => void;
  className?: string;
}

export const ReelPlans = memo(function ReelPlans({ element, onButtonClick, className = '' }: ReelPlansProps) {
  ...
  return (
    <div className={cn('flex flex-col', className)} style={{ gap: `${gap}px` }}>
      {plans.map((plan: any) => {
        ...
        return (
          <div key={plan.id} style={{ ...cardStyle, opacity: cardOpacity }} className="relative">
