...
export default function Index() {
  ...
  const { data: plansData } = useQuery<Plan[]>({
    ...
    queryFn: async () => {
      ...
      return plansArray
        ...
        .filter((plan) => {
          // Ocultar plano gratuito (preço 0) - será exibido separadamente
          return plan.price > 0;
        });
    },
    staleTime: 5 * 60 * 1000,
  ...
  // Buscar plano free separadamente
  const { data: freePlanData } = useQuery<Plan | null>({
    queryKey: ['free-plan-landing'],
    queryFn: async () => {
      const response = await api.getPlans<Plan[]>();
      const plansArray = Array.isArray(response) ? response : [];
      const freePlan = plansArray
        .map((plan) => ({
          ...plan,
          price: normalizePrice(plan.price),
        }))
      ...
      <section id="funnel" className="relative pt-8 pb-8 md:pt-12 md:pb-12 overflow-hidden">
        ...
        {plans.length > 0 && (
          <section id="plans" className="relative py-20 md:py-32">
            ...
            {/* Card horizontal para plano free - abaixo dos outros planos */}
            {freePlanData && (
                ...
                <div className={`relative p-6 md:p-8 rounded-2xl transition-all duration-300 hover:scale-[1.01] group ${
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-8">
                    ...
                    <div className="flex-1 space-y-3">
                      ...
                      {freePlanData.features && Array.isArray(freePlanData.features) && freePlanData.features.length > 0 && (
                        <div className="flex flex-wrap gap-2 md:gap-3 pt-2">
                          {freePlanData.features.map((feature, index) => (
                            <div 
                              key={index} 
